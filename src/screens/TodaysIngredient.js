...
export default function IngredientInfoScreen() {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="pt-14 px-4 space-y-4">
        {/* 뒤로가기 버튼 */}
        <TouchableOpacity
          onPress={() => navigation.goBack()}
          className="absolute top-12 left-5 p-2 rounded-full bg-gr"
        >
          <ChevronLeftIcon size={hp(3.5)} strokeWidth={4.5} color="#fbbf24" />
        </TouchableOpacity>

        <Animated.View entering={FadeInDown.delay(100).duration(600).springify().damping(12)}>
          <Text style={{ fontSize: hp(2.3) }} className="font-semibold text-neutral-700 text-center">
            오늘의 식재료 상식
          </Text>
